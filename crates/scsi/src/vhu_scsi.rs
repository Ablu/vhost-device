...
mod tests {
    ...
    impl<Cb> FakeTarget<Cb> {
        fn new<W: Write, R: Read>(
            collector: Arc<Mutex<FakeTargetCommandCollector>>,
            callback: Cb,
        ) -> Self
        where
            Cb: FnMut(u16, crate::scsi::Request<W, R>) -> FakeResponse + Sync + Send,
        {
            Self {
                collector,
                callback,
    ...
    impl<Cb, W: Write, R: Read> Target<W, R> for FakeTarget<Cb>
    where
        Cb: FnMut(u16, crate::scsi::Request<W, R>) -> FakeResponse + Sync + Send,
    {
        fn execute_command(
